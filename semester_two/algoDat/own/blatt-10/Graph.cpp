...
// gives back the pair of coordinates from this Node
std::pair<float, float> Graph::getNodeCoords(int index) {
    return _nodeCoords[index];
}

// gives back the vector with the NodeConnections from this node to others
std::vector<std::pair<int, float> > Graph::getNodeConnections(int index) {
    return _adjacencyLists[index];
}

// returns number of nodes
size_t Graph::getMaxNodes() {
    return _numNodes;
}
