...
will be covered in \secref{sec:api} and \secref{sec:integration} respectively.
...
% \extend{does it say that it is the same? why is this actually valid?}
...
borrow before the end of the scope, as can be see in \coderef{own2}.
...
non-parallel code (\coderef{par1}) into parallelized code (\coderef{par2}).
...
\tabref{tab:interfaces}, dylib appears to be the best option, which is why it
...
steps before results ended up being \verb|NaN| is described in \coderef{inf1}.
...
should not have changed anything big, however when looking at \coderef{inf2}
...
Note that this test server is different from the one mentioned in \secref{sec:specs}.


\subsection{Parallelizing}

As is demonstrated in \coderef{par1} and \coderef{par2}, the parallelization of
...
As can be seen in \secref{sec:multicore}, this did not really help with
runtimes.
