...
\input{figures/background/Chromatin_Structures}
...
structures are shown in \figref{fig:chromatin}). Looking at it from the outside
...
As can be seen in \figref{fig:cct}, the first steps, cross-linking, digestion,
...
\paragraph{Ligation:}\label{sec:ligation}
...
% This is dealt with differently in the different methods.
...
by adding a second ligation-step (see \secref{sec:ligation} or \figref{fig:cct}
...
for close regions \todo{find source for this}.
...
Biotin during earlier ligation (see \secref{sec:ligation}). Subsequently the
...
As can be seen in \figref{fig:cct} other methods, such as ChIP-loop or ChIA-PET
...
matrices may then be visualized, and other data tracks may be added. This \draft{this is about pyGenomeTracks, add info}
...
visualizations can be seen in \figref{fig:HiCExplorer}. HiCExplorer can be
installed on Linux and Mac OS.

\footnotetext{\url{https://github.com/deeptools/HiCExplorer}, accessed 2019-06-26}

\input{figures/background/figHiCExplorer}



\subsection{Analysis}\label{sec:analysis}

\input{figures/background/HiCMatricesModels}
