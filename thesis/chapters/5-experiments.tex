...
\begin{table}[ht]
    ...
    A third matrix, with a bin length of 10,000 was provided, for which the
    available working memory was not sufficient. Effectively, they all contain the same data.}
    \label{tab:testdata}
\end{table}
...
correspond to the steps described in \secref{sec:ICE}, as the additional bias
...
In the case of the test server (more details in \secref{sec:specs} and
\tabref{tab:specs}) this is the size of an \verb|int64|, whereas coming from
...
As can be seen in \tabref{tab:compmem}, each of the algorithms has some
